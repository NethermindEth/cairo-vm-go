...
func NewRunner(runnerMode RunnerMode, program *Program, hints map[uint64][]hinter.Hinter, proofmode bool, collectTrace bool, maxsteps uint64, layoutName string) (Runner, error) {
...
func (runner *Runner) RunUntilPc(pc *mem.MemoryAddress) error {
	...
	for !runner.vm.Context.Pc.Equal(pc) {
		fmt.Println(runner.steps(), runner.vm.Context.Pc, pc, runner.vm.Context.Fp)
