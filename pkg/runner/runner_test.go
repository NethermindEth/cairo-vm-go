...
func TestSimpleProgram(t *testing.T) {
	...
	runner, err := NewRunner(program, hints, ExecutionModeZero, false, math.MaxUint64, "plain", nil)
...
func TestStepLimitExceeded(t *testing.T) {
	...
	hints := make(map[uint64][]hinter.Hinter)
	runner, err := NewRunner(program, hints, ExecutionModeZero, false, 3, "plain", nil)
	require.NoError(t, err)
...
func TestStepLimitExceededProofMode(t *testing.T) {
	...
	for _, maxstep := range []uint64{6, 7} {
		...
		hints := make(map[uint64][]hinter.Hinter)
		runner, err := NewRunner(program, hints, ProofModeZero, false, uint64(maxstep), "plain", nil)
		require.NoError(t, err)
