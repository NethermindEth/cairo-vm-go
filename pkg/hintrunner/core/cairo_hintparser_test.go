...
func TestCairoHintParser(t *testing.T) {
	t.Run("TestCairoHintParser", func(t *testing.T) {
		...
		starknetProgram, err := starknet.StarknetProgramFromJSON(cairoProgramString)
		require.NoError(t, err)
		output, err := GetCairoHints(starknetProgram)
		require.NoError(t, err)

		require.Equal(t, expectedHintMap, output, "Hint maps do not match")
	})
}
